CLASS zcl_xtt_demo_090 DEFINITION PUBLIC INHERITING FROM zcl_xtt_demo CREATE PUBLIC.
  PUBLIC SECTION.
    ...
    CLASS-METHODS:
      get_one_merge
        IMPORTING
                  io_report       TYPE REF TO zcl_xtt_report
        RETURNING VALUE(rs_merge) TYPE lts_merge.
ENDCLASS.
...
CLASS zcl_xtt_demo_090 IMPLEMENTATION.
  ...
  METHOD get_one_merge.
    ...
    io_report->get_random_table( EXPORTING iv_column_cnt = io_report->mv_c_cnt
...
**********************************
    DATA ls_data TYPE lts_merge.
    ls_data = get_one_merge( mo_report ).
...
**********************************
    mo_report->merge_add_one( is_root    = ls_merge0
                              iv_root_id = 'C' ).
    mo_report->merge_add_one( is_root    = ls_merge1
                              iv_root_id = 'R' ).
  ENDMETHOD.
