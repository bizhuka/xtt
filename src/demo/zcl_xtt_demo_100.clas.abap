...
CLASS zcl_xtt_demo_100 IMPLEMENTATION.
  ...
  METHOD _get_root.
    ...
    " Get icon IDs
    SELECT " id, name + s_raw - For Open ABAP
           * INTO CORRESPONDING FIELDS OF TABLE rs_root-t "#EC TOO_MANY_ITAB_FIELDS
    FROM icon UP TO mo_report->mv_r_cnt ROWS
    WHERE oleng = 2.
    ...
    LOOP AT rs_root-t ASSIGNING <ls_icon>.
      ...
      IF <ls_icon>-s_raw IS NOT INITIAL.
        lv_image = cl_http_utility=>decode_x_base64( <ls_icon>-s_raw ).
      ELSE.
        " Read 1 icon
