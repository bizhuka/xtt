CLASS zcl_xtt_demo_160 DEFINITION PUBLIC INHERITING FROM zcl_xtt_demo_022 CREATE PUBLIC.
...
CLASS zcl_xtt_demo_160 IMPLEMENTATION.
  ...
  METHOD set_merge_info.
    ...
    mo_report->merge_add_one( is_root   = ls_root
                              io_helper = me ).
  ...
  METHOD get_fullname." @see in the template ---> Fullname -> {R@get_fullname( )}
  ...
  METHOD date_text. " @see in the template ---> {R-T@date_text( iv_date = v-FLDATE )}
